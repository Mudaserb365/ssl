...
func processTrustStore(filePath string, config *Config) error {
	...
	if config.NoopMode {
		logNoopAction("process trust store", fmt.Sprintf("%s (Type: %s)", filePath, fileType))
		...
		if config.BaselineURL != "" {
			...
			if err != nil {
				logNoopSkip("comparison failed", filePath)
			}
		}
		return nil
	}
...
func createBackup(filePath string, config *Config) (string, error) {
	...
	// In noop mode, just show what would be done
	if config.NoopMode {
		backupPath := filePath + ".bak." + time.Now().Format("20060102_150405")
		logNoopAction("create backup", fmt.Sprintf("%s -> %s", filePath, backupPath))
